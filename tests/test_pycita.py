...
def test_get_meta_data():
    r = client.get_meta_data()
    assert 'version' in r


def test_get_transaction_receipt_bad():
    # 读取不存在的交易
    assert client.get_transaction_receipt(b'\x00' * 32, 0) == {}
    with pytest.raises(RuntimeError, match='timeout'):
        client.get_transaction_receipt(b'\x00' * 32, 1)

    with pytest.raises(RuntimeError, match='jsonrpc failed'):
        client.get_transaction_receipt(b'\x00')
...
def test_decode_transaction_content():
    ...
    # r2 = client.decode_transaction_content2('')
    # assert r2['transaction']['nonce'] == '2ba194ecf7c04233b8b87d4f359840a1'
...
def test_generate_account():
    pri_key, pub_key, addr = client.signer.generate_account()
    assert pri_key.startswith('0x') and len(pri_key) == 32 * 2 + 2
    assert pub_key.startswith('0x') and len(pub_key) == 64 * 2 + 2
    assert addr.startswith('0x') and len(addr) == 20 * 2 + 2

    # 从私钥还原.
    pri_key2, pub_key2, addr2 = client.signer.generate_account(param_to_bytes(pri_key))
